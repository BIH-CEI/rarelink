...
def map_vital_status(data: dict, processor: DataProcessor) -> VitalStatus:
    ...
    try:
        ...
        highest_instance = get_highest_instance(data.get("repeated_elements", []), "rarelink_3_patient_status")
        ...
        # Map fields
        status_label = processor.fetch_mapping_value("map_vital_status", status_field) if status_field else "UNKNOWN_STATUS"
        time_of_death = processor.process_time_element(time_of_death_field) if time_of_death_field else None
        cause_of_death = (
            OntologyClass(
