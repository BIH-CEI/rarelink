...
def start():
    ...
    if is_admin:
        api_super_token = masked_input(
            "Enter your super API token (input will be masked, or press Enter to skip): ",
            mask="#",
        )
        typer.echo()
    ...
    typer.echo(f"▶ Run {format_command('rarelink redcap-setup api-config view')} to see the current configuration.")
    typer.echo(f"▶ If you had a typo, you can run {format_command('rarelink redcap-setup api-config start')} again.")
    typer.echo(f"▶ Run {format_command('rarelink redcap-setup api-config reset')} to reset the configuration.")
    hint_text(
