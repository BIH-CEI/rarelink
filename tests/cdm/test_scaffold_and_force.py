...
def test_scaffold_force_overwrite(tmp_path: Path, monkeypatch):
    ...
    (prev / "datamodel" / "__init__.py").write_text("# old init", encoding="utf-8")
    (prev / "__init__.py").write_text("# old top init", encoding="utf-8")

    # First run: v2_0_4 from v2_0_4
    res1 = scaffold_version_package("v2_0_4", root, from_version="v2_0_4", force=False)  # noqa: F841
    assert (root / "v2_0_4").exists()
    assert (root / "v2_0_4" / "schema_definitions" / "rarelink_code_systems.yaml").exists()
    assert (root / "v2_0_4" / "datamodel" / "rarelink_code_systems.py").exists()

    # Second run with force should delete and recreate cleanly
    res2 = scaffold_version_package("v2_0_4", root, from_version="v2_0_4", force=True)  # noqa: F841
    assert (root / "v2_0_4" / "datamodel" / "rarelink_code_systems.py").exists()
