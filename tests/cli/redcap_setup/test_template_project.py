...
@patch("rarelink.cli.redcap_setup.template_project.CONFIG_FILE", "/tmp/mock_config.json")
...
@patch("requests.post")
def test_template_project_with_super_token(mock_post, mock_file):
    """
    Test the `template_project` command when the user has a super token.
    """
    ...
    result = runner.invoke(app, ["template-project"], input="yes\n")
...
def test_template_project_without_super_token(mock_file):
    """
    Test the `template_project` command when the user does not have a super token.
    """
    result = runner.invoke(app, ["template-project"], input="no\n")
    assert result.exit_code == 0
    ...
    assert "rarelink redcap-setup download template_project" in result.stdout

@patch("rarelink.cli.redcap_setup.template_project.CONFIG_FILE", "/tmp/mock_config.json")
@patch("builtins.open", new_callable=mock_open, read_data='{}')
def test_template_project_missing_config(mock_file):
    """
    Test the `template_project` command when the configuration file is missing.
    """
    result = runner.invoke(app, ["template-project"], input="yes\n")
...
def test_template_project_missing_super_token(mock_file):
    """
    Test the `template_project` command when the super token is missing.
    """
    result = runner.invoke(app, ["template-project"], input="yes\n")
    assert result.exit_code == 1
